...
def maximin_fitness(designidx, swarm_particle):
    # Take in a design and calculate the maximin score against the population of designs
    population = len(swarm_particle)
    avg_stress, max_displace = get_stress_displace(swarm_particle[designidx].particle_position, False)
    # swarm_particle[designidx].evaluate(objective_function)
    designfit = [avg_stress, max_displace]
    numobj = len(designfit)
    
    # Calculate the fitnesses and fill an array of fitness values
    popfitnesses = []
    for i in range(population):
        if i != designidx:
            avg_stress, max_displace = get_stress_displace(swarm_particle[i].particle_position, False)
            popfitnesses.append([avg_stress, max_displace])
        
    minvals = np.zeros(((population-1),numobj))
    for i in range(population-1):
        for j in range(numobj):
            minvals[i,j] = designfit[j] - popfitnesses[i][j]
            
    mins = np.zeros(population-1)
    for i in range(population-1):
        mins[i] = np.min(minvals[i,:])
...
if __name__ == "__main__":
    ...
    optimize_particle_swarm(mm, bounds, particle_size, iterations, w, c1, c2)